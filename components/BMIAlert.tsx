...
export function BMIAlert({
  ...
  const [showVideo, setShowVideo] = useState(
    autoPlayVideo && threshold.autoPlay
  );
      ...
      >
        ...
        <CardHeader className="pb-4">
          <div className="flex items-start gap-4">
            <div
              className={`p-3 rounded-full ${colors.bg} border ${colors.border}`}
            >
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="text-center p-2 bg-white/70 rounded-lg">
                  <div
                    className={`text-sm font-semibold ${colors.text} capitalize`}
                  >
                ...
                <div className="flex items-center gap-3 p-3 bg-white/80 rounded-lg">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800">
                      {weightDifference > 0
                        ? `Se recomienda perder ${Math.abs(
                            weightDifference
                          ).toFixed(1)} ${unit}`
                        : `Se recomienda ganar ${Math.abs(
                            weightDifference
                          ).toFixed(1)} ${unit}`}
                    </p>
                    <p className="text-xs text-slate-600">
                      Peso ideal: {idealWeight.min.toFixed(1)} -{" "}
                      {idealWeight.max.toFixed(1)} {unit}
                    </p>
                  </div>
                </div>
              )}
